...
def normalize_devanagari_spacing(
    ...
    *,
    ...
    while index < len(text):
        if text[index] == " ":
            ...
            if (
                ...
                remove = True
            # 🛑 A space after a virama, and ONLY while markers are being resolved.
            #
            # Both readings of this are right about a different case and neither rule alone
            # is sufficient, because the two are orthographically identical: `सञ् चालन` is
            # one word a span split in half, and `छन् तथा` is two words with a real space,
            # and both are "space after virama". What separates them is not the text but
            # the pass. The marker-resolution pass is fed text reassembled from spans, so a
            # space there is a splitting artifact; ordinary text has authored spaces, and a
            # blanket deletion joined them -- which is why the narrow `पुर्याउनु`
            # substitution below exists instead.
            #
            # So the deletion is scoped to `preserve_marker_spaces`, the flag that says
            # this is that pass. `_PUA_CONTEXTUAL_NE` stays excluded: its space is the only
            # thing recording where the marker's base ended.
            if (
                preserve_marker_spaces
                and not protected_boundary
                and result
                and result[-1] == _VIRAMA
                and next_char != _PUA_CONTEXTUAL_NE
            ):
                remove = True
            if remove:
                index += 1
    ...
    #
    # 🛑 The extractor line these Kokila changes come from applied that rule to EVERY pass.
    # It is scoped to the marker pass above instead, which is what lets both cases be right;
    # the reasoning is at that branch. Restoring the unscoped form would re-join the word
    # boundaries this comment names.
    #
    # ⚠️ Worth knowing how this was nearly got wrong: no test in that line calls this
    # function directly, so it read as uncovered and dropping the rule outright looked
    # free. Two marker tests reach it end to end and failed on it -- a grep for direct
    # callers is not a coverage measurement.
