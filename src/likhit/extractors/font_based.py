    ...
    def _convert_span_text(
        ...
        if content_legacy_maps and not skip_content_legacy:
            ...
            if content_choice is not None and content_choice.map_key is not None:
                ...
                #
                # ⚠️ VOL-515 splits the two arguments, and the split is deliberate:
                # the veto reads `decided_on` (the whole SPAN under the exemption
                # splice, `text` otherwise) while the decode writes `unmarked` (this
                # SEGMENT). So the "one string" property above holds per-decision
                # rather than per-call, which is the point -- a segment must not
                # answer a span-calibrated veto differently from its span.
                # Passing the marked `decided_on` costs nothing, because
                # `_reads_as_latin_words` unmarks on its own first line with the
                # WIDER `unmark_cids`; wrapping it in `unmark_cid_ascii` first is
                # provably the same value (the narrow form's output is ASCII, which
                # the wide form then leaves alone), so this is not the veto silently
                # losing VOL-159's unmark.
