...
class DocxBasedStrategy(ExtractionStrategy):
    ...
    def _extract_docx(self, file_path: str) -> str:
        """Extract plain text from DOCX file using docx2txt2."""
        try:
            # docx2txt2.process() extracts text including tables
            text = docx2txt2.process(file_path)
            return text if text else ""
        except Exception as e:
            raise ExtractionError(f"Failed to extract text from DOCX: {e}") from e

    def _extract_doc(self, file_path: str) -> str:
        """Extract plain text from legacy DOC file using pyantiword.

        Note: pyantiword may not work on Windows due to binary compatibility issues.
        """
        try:
            ...
            # Windows compatibility issue with antiword binary
            if "Win32" in str(e) or "WinError" in str(e):
                raise ExtractionError(
                    f"Failed to extract text from DOC: pyantiword is not compatible with Windows. "
                    f"Please convert the file to DOCX format first, or use a Linux/Mac system. "
                    f"Original error: {e}"
                ) from e
